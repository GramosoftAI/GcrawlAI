...
def safe_get_content(page: Page, timeout_ms: int = 5000) -> str:
    """
    Safely retrieve page content, waiting for active navigation to complete if needed.
    Prevents crash if page is in transient state.
    """
    try:
        return page.content()
    except Exception as e:
        if "navigating" in str(e).lower():
            logger.info("Page is currently navigating/redirecting. Waiting for load state to stabilize...")
            try:
                page.wait_for_load_state("load", timeout=timeout_ms)
                return page.content()
            except Exception as wait_err:
                logger.warning(f"Error waiting for page load stabilization: {wait_err}")
                try:
                    return page.content()  # Try one last time
                except Exception:
                    return ""
        raise e

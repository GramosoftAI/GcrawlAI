    ...
    def process_page(
        ...
        try:
            ...
            if is_status_blocked and not triggered_reason:
                ...
                is_sparse = len(stripped_content) < 150 and len(links) < 5
                
                if not is_block_title and not is_sparse:
                    is_status_blocked = False
                    logger.info(f"Page returned status code {status_code} but content is fully populated and title is valid. Bypassing status block check.")
                else:

...
def _build_proxy_config(session_id: Optional[str] = None) -> Dict:
    ...
    # Enable sticky IP session by appending _session-<id> to the Evomi password
    # This prevents CAPTCHAs by keeping IP consistency within the page load,
    # and isolates concurrent parallel queries on separate premium IPs!
    if session_id and pw and "evomi" in (server or "").lower():
        if "_session-" not in pw:
            pw = f"{pw}_session-{session_id}"
...
async def scrape_google(query: str, limit: int = 10, ip: Optional[str] = None, **kwargs) -> List[Dict]:
    ...
    if sys.platform == 'win32':
        ...
        if is_selector:
            ...
            future = asyncio.run_coroutine_threadsafe(search(query, limit), target_loop)
            try:
...
async def search(query: str, limit: int = 10) -> Dict:
    ...
    async with sem:
        ...
        target_pages = (limit + 7) // 8
        redundancy = 2 if limit > 10 else 1
        num_pages = min(target_pages + redundancy, 10)  # Max 10 parallel pages to protect local system resources
        starts = [i * 10 for i in range(num_pages)]
        if len(starts) < 2:
            starts = [0, 10]
            
        logger.info(f"[Thread-{tid}] Fetching Google starts: {starts} in parallel for limit={limit}")
        
        # Borrow browsers in parallel from the pool
        fetchers = await asyncio.gather(*[_pool.borrow() for _ in starts])
        try:
            # Throttle
            await _apply_provider_throttle("evomi")
            
            async def fetch_page(fetcher, start: int):
                url = f"https://www.google.com/search?q={query.replace(' ', '+')}&start={start}"
                session_id = "".join(random.choices("0123456789abcdef", k=8))
                fetcher.proxy = _build_proxy_config(session_id=session_id)
                try:
            ...
            tasks = [fetch_page(fetchers[i], starts[i]) for i in range(len(starts))]
            results_and_responses = await asyncio.gather(*tasks)
            ...
            for res_list, resp in results_and_responses:
                if resp:
                    valid_response = resp
                for r in res_list:
                    url = r.get("url")
            ...
            # Return all fetchers back to the pool
            await asyncio.gather(*[_pool.return_fetcher(f) for f in fetchers])

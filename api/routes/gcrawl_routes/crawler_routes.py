...
from api.models.gcrawl_payloads import ScrapeRequest, MultiCrawlRequest, LinksRequest, ScreenshotRequest, CrawlResponse, OfflineBundleRequest
...
async def run_scrape(
    ...
    """Run scrape."""
    try:
        if payload.screenshot and payload.screenshot.enabled and payload.screenshot.js_render:
        ...
        try:
            ...
            if raw_url and isinstance(raw_url, str):
                ...
                if raw_url.rstrip("/") == start_url.rstrip("/"):
                    start_url = raw_url
        except Exception:
            pass
        ...
        with get_pooled_connection() as conn:
            ...
            conn.commit()
            cur.close()
        log_activity(user_id, "/SCRAPE", start_url, "COMPLETED", job_id=crawl_id)
        return {
            "status_code": 200,
            "crawl_id": crawl_id,
        ...
        try:
            ...
            log_activity(user_id, "/SCRAPE", start_url, "FAILED", job_id=crawl_id if 'crawl_id' in locals() else None)
        except Exception:
            pass
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/scrape/offline-bundle", response_model=CrawlResponse)
async def run_offline_bundle(
    ...
    try:
        ...
        from api.core.security import check_plan_limits_and_get_details, increment_used_requests, check_endpoint_active
        check_endpoint_active("Offline Bundle API")
        plan_type, concurrency_limit = check_plan_limits_and_get_details(user_id)
        ...
        config.proxy_geo = payload.proxy.geo if payload.proxy else None
        ...
        def _wrapper(**kw):
            """Wrapper for offline zipping."""
            from web_crawler.crawler.helpers.crawler_offline import generate_offline_bundle
            try:
                ...
                logger.error(f"Error running offline bundle background job: {e}", exc_info=True)
                from api.core.database import update_activity_log_status
                    ...
                    with get_pooled_connection() as conn:
                        with conn.cursor() as cursor:
                            cursor.execute(
                                ...
                                VALUES (%s, %s, %s, %s, %s, %s)
                                """,
                                (crawl_id, str(payload.url), "Offline Bundle Scraper", str(e), str(e), str(user_id) if user_id else "1")
                            )
                        conn.commit()
                ...
                try:
                    ...
                    log_admin_error(
                        ...
                        proxy_ip="Proxy/Direct Escalated",
        ...
        log_activity(user_id, "/OFFLINE-BUNDLE", str(payload.url), "COMPLETED", job_id=crawl_id)
        ...
        try:
            ...
            log_activity(user_id, "/OFFLINE-BUNDLE", str(payload.url), "FAILED", job_id=crawl_id if 'crawl_id' in locals() else None)
        except Exception:
            pass
        raise HTTPException(status_code=500, detail=str(e))
...
async def run_crawl(
    ...
    try:
        ...
        log_activity(user_id, "/CRAWL", start_url, "COMPLETED", job_id=crawl_id)
        return {
            "status_code": 200,
            "crawl_id": crawl_id,
            "polling_url": f"https://gcrawlai.com/gc/crawler/data/{crawl_id}",

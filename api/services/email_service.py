...
class EmailService:
    ...
    def __init__(self, smtp_config: Dict[str, Any]):
        ...
        self.use_tls = str(smtp_config.get('use_tls', True)).lower() in ('true', '1', 't', 'yes')
    ...
    def send_email(self, to_email: str, subject: str, 
        ...
        try:
            ...
            # Add HTML version (Initial template for single-recipient logic if needed elsewhere)
            msg.attach(MIMEText(html_content, 'html', 'utf-8'))
            
            # Dispatch individual emails to each recipient
            ...
            for recipient in recipients:
                try:
                    ...
                    # ✅ Each recipient gets its own fresh SMTP connection
                    ...
                    logger.error(f"✗ SMTP auth failed sending to {recipient}: {e}")
                except smtplib.SMTPException as e:
                    logger.error(f"✗ SMTP error sending to {recipient}: {e}")
                except Exception as e:
                    logger.error(f"✗ Failed to send email to {recipient}: {e}")

            logger.info(f"Email dispatch summary: {success_count}/{len(recipients)} sent")
            return success_count > 0
            
        except Exception as e:
            logger.error(f"✗ Fatal error in email dispatch: {e}")
            return False
